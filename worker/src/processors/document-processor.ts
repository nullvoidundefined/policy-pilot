...
export async function processDocument(job: Job<DocumentProcessJob>): Promise<void> {
  ...
  try {
    ...
    if (text.trim().length === 0) {
      await updateStatus(documentId, "failed", { error: "No text content found in document" });
      return;
    }
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await query(
        ...
        [documentId, userId, chunk.index, chunk.content, chunk.tokenCount, embeddingStr],
      );
    }
